...
public class ClasroomAdapter extends RecyclerView.Adapter<ClassViewHolder> {
    ...
    public void joinClassroom(String codeClass, String userId) {
        DatabaseReference joinRef = FirebaseDatabase.getInstance().getReference("users_classrooms");
                ...
                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                    ...
                    if (classroom != null && classroom.getCodeClass().equals(codeClass)) {
                        ...
                        if (newId != null) {
                            ...
                            joinRef.child(newId).child("idUser").setValue(userId);
                        }
                    }
                }
            }
    ...
    public void insertClassroom(String classname, String userId) {
        DatabaseReference classRef = FirebaseDatabase.getInstance().getReference("classrooms");
        String newId = classRef.push().getKey();
        String generatedCode = generateClassCode();

        if (newId != null) {
            classroom classroom = new classroom(newId, classname, generatedCode, userId, "1", "teacher");
    ...
    public void deleteClasroom(classroom classroom) {
        ...
        builder.setTitle("Xác nhận xóa")
                ...
                .setPositiveButton("Xóa", (dialog, which) -> {
                    DatabaseReference myRef = FirebaseDatabase.getInstance().getReference("classrooms").child(classroom.getIdClass());
                    myRef.removeValue()
                            .addOnSuccessListener(aVoid ->
